...
SUMMARY_METRIC_ORDER = [
    ...
    "clat_p99_00_ms",
    "clat_p99_90_ms",
    "clat_p99_99_ms",  # Dynamically generated keys
    "cpu_usr_pct",
    "cpu_sys_pct",
    "disk_util_pct",
    "duration_sec",
...
def generate_comparison_summary(results1, results2, label1, label2):
    ...
    summary_metrics = []
    base_summary_order = ["iops", "bandwidth_mibps", "clat_avg_ms", "clat_max_ms"]
    percentile_summary_order = sorted(
        [
            f"clat_p{p.replace('.', '_')}_ms"
            for p in PERCENTILES_TO_AGGREGATE
            if f"clat_p{p.replace('.', '_')}_ms" in results1
            or f"clat_p{p.replace('.', '_')}_ms" in results2
        ]
    )
    ...
    for metric in summary_metrics:
        ...
        def format_res(res):
            if res:
                ...
                if "ms" in metric:
                    precision = 3
                elif "pct" in metric or "util" in metric:
                    precision = 2
                elif "iops" in metric:
                    precision = 1
                elif "mibps" in metric:
                    precision = 1
                else:
                    precision = 2  # Default
        ...
        metric_label = (
            metric_label.replace(" iops", " IOPS")
            .replace(" cpu", " CPU")
            .replace(" disk", " Disk")
            .title()
        )
...
def plot_fio_comparison(agg_data1, agg_data2, label1, label2):
    ...
    # --- Plot 1: IOPS ---
    metric_key = "iops"
    title = f"Fio IOPS Comparison ({operation})"
    ylabel = "IOPS"
    ...
    if d1_data and d2_data:
        ...
        ax.set_ylim(
            bottom=0, top=max(means1 + means2) * 1.15 if max(means1 + means2) > 0 else 1
        )
        save_plot(fig, "fio_comparison_iops")
    else:
        print(f"   Skipping {ylabel} plot: Missing data.")

    # --- Plot 2: Bandwidth ---
    metric_key = "bandwidth_mibps"
    title = f"Fio Bandwidth Comparison ({operation})"
    ...
    # --- Plot 3: Latency ---
    latency_metrics = ["clat_avg_ms"] + sorted(
    ...
    if plot_resource_labels:
        ...
        )  # Ensure some space, max maybe 100
    ...
    if plot_labels_p:
        ...
        ax_p.set_title(
            f"Fio Performance % Difference ({label2_short} vs {label1_short})"
        )
