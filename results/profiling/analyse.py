...
if not csv_files:
    ...
    for i, file_path in enumerate(csv_files):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  Warning: File {file_path} contains non-numeric data. Skipping max time calculation."
                )
                continue
            ...
            if numeric_df_pass1.empty:
                print(
                    f"  Warning: No numeric data found in {file_path}. Skipping max time calculation."
                )
                continue

            # --- Sampling (Consistent with second pass) ---
            if len(numeric_df_pass1) > SAMPLE_SIZE:
                numeric_df_pass1 = numeric_df_pass1.sample(
            ...
            if total_times_pass1.empty:  # Check if sampling resulted in empty df
                print(
                    f"  Warning: Sampling resulted in empty data for {file_path}. Skipping max time calculation."
                )
                continue
            ...
            clipped_times_pass1 = total_times_pass1[
                (total_times_pass1 >= lower_bound_pass1)
                & (total_times_pass1 <= upper_bound_pass1)
            ]
            ...
            if not clipped_times_pass1.empty:
                ...
                print(
                    f"  Warning: No data left after clipping in {file_path}. Skipping max time update."
                )
    ...
    print(
        f"\n--- Global Maximum Clipped System Call Time: {global_max_time_clipped:.2f} ns ---"
    )
    if global_max_time_clipped == 0:
        print(
            "Warning: Global max time is 0. Plots might have unexpected x-axis limits."
        )
    ...
    for i, file_path in enumerate(csv_files):
        ...
        try:
            ...
            if df.empty:
                print(
                    f"Warning: File {file_path} is empty or contains only headers. Skipping."
                )
                continue
            try:
                ...
                print(
                    f"Error: File {file_path} contains non-numeric data that cannot be analyzed numerically. Skipping."
                )
                print(f"Details: {e}")
                continue
            ...
            if not means_df.empty:
                ...
                if header_order:
                    try:
                        ...
                        print(
                            f"  Warning: Could not reindex means for {file_path} to header order. Error: {reindex_e}"
                        )
            ...
            numeric_df_plot = numeric_df_full.select_dtypes(
                include=np.number
            ).copy()  # Work on a copy
            ...
            if num_rows_original > SAMPLE_SIZE:
                ...
                numeric_df_plot = numeric_df_plot.sample(
                    n=SAMPLE_SIZE, random_state=RANDOM_SEED
                )
                is_sampled = True
            else:
            ...
            print(
                "Clipping outliers based on total time per system call (IQR method)..."
            )
            total_times = numeric_df_plot.sum(axis=1)
            if total_times.empty:  # Check if sampling resulted in empty df
                print(
                    f"  Warning: Sampling resulted in empty data for {file_path}. Skipping plot generation."
                )
                continue
            ...
            numeric_df_plot = numeric_df_plot[
                (total_times >= lower_bound) & (total_times <= upper_bound)
            ]
            clipped_count = original_count - len(numeric_df_plot)
            # Avoid division by zero if original_count was 0 (though unlikely here)
            percentage_clipped = (
                (clipped_count / original_count * 100) if original_count > 0 else 0
            )
            print(
                f"  Removed {clipped_count} outliers ({percentage_clipped:.1f}% of plotted data)."
            )
            ...
            if numeric_df_plot.empty:
                print(
                    f"Warning: No data left after outlier clipping. Skipping time breakdown plot for {file_path}."
                )
            ...
            numeric_df_plot.plot(
                kind="barh", stacked=True, ax=ax1, width=0.8
            )  # Use width for bar thickness

            # Customize the plot
            ax1.set_xlabel("Time (ns)")  # Updated label
            ax1.set_ylabel("Sampled System Call Index (Post-Clipping)")  # Updated label
            plot_title = f"System Call Time Breakdown - {os.path.basename(file_path)}"
            if is_sampled:
                plot_title = f"Sampled System Call Time Breakdown - {os.path.basename(file_path)}"
            ax1.set_title(plot_title + " (Clipped)")  # Add clipped indicator

            # Place legend outside the plot area
            ax1.legend(
                title="Filtering Stage", bbox_to_anchor=(1.05, 1), loc="upper left"
            )
            ...
            ax1.set_xlim(
                0,
                (
                    global_max_time_clipped * X_AXIS_PADDING
                    if global_max_time_clipped > 0
                    else None
                ),
            )  # Avoid setting limit if max is 0

            # --- Add logic to hide y-axis ticks if too many bars ---
            if num_rows_to_plot > Y_AXIS_TICK_THRESHOLD:
                print(
                    f"Hiding y-axis tick labels for {num_rows_to_plot} bars to improve performance."
                )
            ...
            if is_sampled:
                time_plot_filename = (
                    f"{base_filename}_time_breakdown_sampled_clipped.pdf"
                )
            ...
            print(
                f"An unexpected error occurred while processing {file_path}: {e}. Skipping."
            )
    ...
    if not all_means_data:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  Warning: Could not reindex DataFrame stages to header order. Plotting with potentially incorrect stage order. Error: {reindex_e}"
                )
            ...
            # Customize the plot
            ax_means.set_title("Mean System Call Time Breakdown Across Files")
            ax_means.set_ylabel("File")  # Files are now on the y-axis
            ax_means.set_xlabel(
                "Total Mean Time (ns)"
            )  # x-axis is the stacked total time
            ...
            # Save the plot
            mean_plot_filename = (
                "mean_stacked_horizontal_comparison.pdf"  # Updated filename
            )
