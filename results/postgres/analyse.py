...
def parse_pgbench_log(filepath):
    ...
    try:
        ...
        if match:
            metrics["clients"] = int(match.group(1))
        ...
        if match:
            metrics["duration_s"] = int(match.group(1))
        match = re.search(
            r"number of transactions actually processed:\s+(\d+)", content
        )
        if match:
            metrics["transactions_processed"] = int(match.group(1))
        match = re.search(
            r"number of failed transactions:\s+(\d+)\s+\(([\d.]+)%\)", content
        )
        if match:
        ...
        if match:
            metrics["latency_avg_ms"] = float(match.group(1))
        match = re.search(r"initial connection time\s*=\s*([\d.]+)\s+ms", content)
        if match:
            metrics["initial_conn_time_ms"] = float(match.group(1))
    ...
    if metrics["tps"] is None or metrics["latency_avg_ms"] is None:
        if not metrics["error"]:
            metrics["error"] = (
                f"Could not extract key pgbench metrics (TPS/Latency) from '{os.path.basename(filepath)}'"
            )
    return metrics
...
def aggregate_pgbench_results(all_metrics_data):
    ...
    if valid_run_count == 0:
        return None
    query_modes = set(
        run_data.get("query_mode")
        for run_data in all_metrics_data
        if run_data and run_data.get("query_mode")
    )
    if len(query_modes) == 1:
        aggregated_results["query_mode"] = {
            "value": query_modes.pop(),
            "consistent": True,
            "count": valid_run_count,
        }
    ...
    if not any(key in aggregated_results for key in ["tps", "latency_avg_ms"]):
        return None
...
def generate_comparison_summary(results1, results2, label1, label2):
    ...
    for metric in config_items:
        ...
        if res1:
            count1 = res1["count"]
            consistent1 = (
                res1.get("consistent", False)
                if not is_str_val
                else res1.get("consistent", False)
            )
            if is_str_val:
                res1_str = f"{res1['value']} (n={count1})" + (
                    " (consistent)"
                    if consistent1
                    else (" (!)" if not consistent1 and count1 > 0 else "")
                )
            elif res1.get("mean") is not None:
                is_int = isinstance(res1["mean"], int) or metric in [
                    "clients",
                    "threads",
                    "scaling_factor",
                    "duration_s",
                ...
                if consistent1:
                    res1_str = f"{res1['mean']:.{precision}f} (n={count1}, consistent)"
                else:
                    res1_str = f"{res1['mean']:.{precision}f} +/- {res1['stdev']:.{precision}f} (n={count1})"

        if res2:
            count2 = res2["count"]
            consistent2 = (
                res2.get("consistent", False)
                if not is_str_val
                else res2.get("consistent", False)
            )
            if is_str_val:
                res2_str = f"{res2['value']} (n={count2})" + (
                    " (consistent)"
                    if consistent2
                    else (" (!)" if not consistent2 and count2 > 0 else "")
                )
            elif res2.get("mean") is not None:
                is_int = isinstance(res2["mean"], int) or metric in [
                    "clients",
                    "threads",
                    "scaling_factor",
                    "duration_s",
                ...
                precision = 0 if is_int else 2
                if consistent2:
                    res2_str = f"{res2['mean']:.{precision}f} (n={count2}, consistent)"
                else:
                    res2_str = f"{res2['mean']:.{precision}f} +/- {res2['stdev']:.{precision}f} (n={count2})"

        summary_lines.append(
            col_header_format.format(
                f"  {metric.replace('_',' ').title()}", res1_str, res2_str, ""
            )
        )  # No %diff for config
    ...
    for metric in SUMMARY_METRIC_ORDER:
        ...
        def format_res(res):
            if res and res.get("mean") is not None:
                ...
                precision = (
                    0
                    if is_int
                    else (3 if "latency" in metric or "pct" in metric else 2)
                )
        ...
        summary_lines.append(
            col_header_format.format(
                f"  {metric_label}", res1_str, res2_str, percent_diff_str
            )
        )
...
def plot_pgbench_comparison(agg_data1, agg_data2, label1, label2):
    ...
    def create_single_metric_plot(
        metric_key, title, ylabel, filename_suffix, precision=2
    ):
        ...
        if d1_data and d2_data:
            ...
            ax.bar_label(r1, padding=3, fmt=fmt_str)
            ax.bar_label(r2, padding=3, fmt=fmt_str)
            ax.set_ylim(
                bottom=0,
                top=max(means1 + means2) * 1.15 if max(means1 + means2) > 0 else 1,
            )
            save_plot(fig, f"pgbench_comparison_{filename_suffix}")
        else:
            print(f"   Skipping plot for '{metric_key}': Missing data.")

    # --- Create the individual absolute value plots ---
    create_single_metric_plot(
        "tps", "Comparison of Transactions Per Second", "TPS", "tps", precision=1
    )
    create_single_metric_plot(
        "latency_avg_ms",
    ...
    create_single_metric_plot(
        "initial_conn_time_ms",
        "Comparison of Initial Connection Time",
        "Time (ms)",
        "conn_time",
        precision=3,
    )
    ...
    for metric in transaction_metrics:
        ...
        if d1_txn and d2_txn:
            ...
            plot_txn_labels.append(
                metric.replace("transactions_", "")
                .replace("_count", "")
                .replace("_", " ")
                .title()
            )
    ...
    if plot_txn_labels:
        ...
        ax_txn.bar_label(r1, padding=3, fmt="%.0f")
        ax_txn.bar_label(r2, padding=3, fmt="%.0f")
        ax_txn.set_ylim(bottom=0)
        if ax_txn.get_yscale() != "log":
            ax_txn.set_ylim(
                top=(
                    max(means1_txn + means2_txn) * 1.15
                    if max(means1_txn + means2_txn) > 0
                    else 1
                )
            )
        ...
        print(
            f"   Skipping Transactions plot: No common transaction metrics with data found."
        )
    ...
    if plot_labels_p:
        ...
        ax_p.set_title(
            f"pgbench Performance % Difference ({label2_short} vs {label1_short})"
        )
