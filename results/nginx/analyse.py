...
def generate_comparison_summary(results1, results2, label1, label2):
    ...
    for metric in config_items:
        ...
        res2_str = "N/A"
        if res1:
            res1_str = f"{int(res1['mean'])} (n={res1['count']})" + (
                "" if res1.get("consistent") else " (!)"
            )
        if res2:
            res2_str = f"{int(res2['mean'])} (n={res2['count']})" + (
                "" if res2.get("consistent") else " (!)"
            )
        summary_lines.append(
            col_header_format.format(metric.title(), res1_str, res2_str, "")
        )  # No %diff for config
    ...
    for metric in summary_metric_order:
        if metric in config_items:
            continue  # Skip config items already handled
        ...
        def format_res(res):
            if res:
                precision = (
                    3 if "latency" in metric else (0 if "requests" in metric else 2)
                )
        ...
        if "_ms" in metric:
            unit = "ms"
        elif "req_sec" in metric:
            unit = "req/s"
        elif "_mb" in metric:
            unit = "MB"
        elif "transfer" in metric:
            unit = "MB/s"
        elif "_s" in metric:
            unit = "s"

        metric_label = (
            f"{metric.replace('_', ' ').title()} ({unit})"
            if unit
            else metric.replace("_", " ").title()
        )
        summary_lines.append(
            col_header_format.format(metric_label, res1_str, res2_str, percent_diff_str)
        )
...
def plot_comparison(agg_data1, agg_data2, label1, label2):
    ...
    if d1_rps_data and d2_rps_data:
        ...
        ax.set_xlabel("Requests per Second (Overall)")
        ax.set_title("Comparison of Overall Requests per Second")
        ax.set_yticks(y_pos)
        ...
        ax.set_xlim(
            right=max(means1 + means2) * 1.15 if max(means1 + means2) > 0 else 1
        )
        fig.tight_layout()
        plot_filename = os.path.join(
            output_plot_dir, "wrk_comparison_requests_per_second.pdf"
        )
        try:
        ...
        print(f"   Skipping plot for '{metric_rps}': Missing data.")
    ...
    if plot_latency_labels:
        ...
        ax.set_title("Comparison of Latency Metrics")
        ...
        ax.set_ylim(
            bottom=0,
            top=(
                max(means1_lat + means2_lat) * 1.15
                if max(means1_lat + means2_lat) > 0
                else 1
            ),
        )
        fig.tight_layout()
    ...
    d1_transfer, d2_transfer = agg_data1.get(metric_transfer), agg_data2.get(
        metric_transfer
    )
    if d1_transfer and d2_transfer:
        means1 = [d1_transfer["mean"]]
        stdevs1 = [d1_transfer["stdev"]]
        means2 = [d2_transfer["mean"]]
        stdevs2 = [d2_transfer["stdev"]]
        y_pos = np.arange(len(means1))
        bar_height = 0.35
        fig, ax = plt.subplots(figsize=(8, 4))
        rects1 = ax.barh(
        ...
        ax.set_xlabel("Transfer Speed (MB/s)")
        ax.set_title("Comparison of Overall Transfer Speed")
        ax.set_yticks(y_pos)
        ax.set_yticklabels(["Transfer"])
        ax.invert_yaxis()
        ...
        fig.tight_layout()
        plot_filename = os.path.join(
            output_plot_dir, "wrk_comparison_transfer_speed.pdf"
        )
        try:
            plt.savefig(plot_filename, format="pdf", bbox_inches="tight")
            print(f"   Plot saved to '{plot_filename}'")
        ...
        print(f"   Skipping plot for '{metric_transfer}': Missing data.")
    ...
    if plot_labels_p:
        ...
        colors = [
            "red" if x < 0 else "green" for x in percent_diffs
        ]  # Color bars by sign
        ...
        print(
            f"   Skipping Percentage Difference plot: No common metrics with data for comparison."
        )
